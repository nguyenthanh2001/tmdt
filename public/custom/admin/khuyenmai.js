$(document).ready(function () {
    $('#formthemkhuyenmai').on('submit', function (event) {
        ...
        $.ajax({
           ...
           success: function (data2) {            
             ...
             if (trangthai == true) {               
               toastr.success('Thêm khuyến mãi thành công', 'Thông báo');  
               loaddata();            
             }
             else{
               toastr.error('Thêm thất bại thất bại',"Thông báo")
               setTimeout(function(){
                location.reload();
            }, 1000);
             }
        ...
        $('#form-edit').on('submit', function (event) {
            ...
            var url_form_edit = 'edit-khuyen-mai/'+id;
            $.ajax({
               url : url_form_edit,
               method: 'POST',
               data: new FormData(this),
               ...
               success: function (data2) {              
                 var trangthai=data2.status;
                 if (trangthai == true) {               
                   toastr.success('Sửa khuyến mãi thành công', 'Thông báo');                         
                   loaddata();            
                 }
                 else{
                   toastr.error('Sửa khuyến mãi thất bại',"Thông báo")
                   setTimeout(function(){
                    location.reload();
                }, 1000);
                 }                 
...
function loaddata(){
  ...
  function suakm(id) {
    var url_edit='edit-khuyen-mai/'+id;
    $.ajax({
      url : url_edit,
      method: 'GET',
      contentType: false,
  ...
  function xoakm(id) {
    var url_delete = 'delete-khuyen-mai/'+id;
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        ...
        success: function (data2) {              
          var trangthai=data2.status;
          if (trangthai == true) {               
            toastr.success('Xóa khuyến mãi thành công', 'Thông báo');                         
            loaddata();            
          }
          else{
            toastr.error('Không thể xóa khuyến mãi này',"Thông báo")
         }                 
        },
         error: function (data_er) { 
             toastr.warning(data_er.responseJSON.message,"Thông báo") 
